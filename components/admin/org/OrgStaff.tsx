...
export default function OrgStaff({ orgId, org, onSaved }:{ orgId: string; org: Org; onSaved: (o: Org)=>void; }){
    ...
    <div className="grid gap-4">
      <div className="flex gap-2 items-end">
        <label className="flex-1 grid gap-1">
          ...
          <input type="email" placeholder="name@company.com" value={email} onChange={e=>setEmail(e.target.value)} className="border rounded px-3 py-2 bg-white dark:bg-slate-900"/>
        </label>
        <button onClick={invite} disabled={busy || !email} className="px-4 h-10 rounded bg-blue-600 text-white disabled:opacity-50">Invite</button>
      </div>
        ...
        <ul className="divide-y">
          ...
          {users.map(u => (
            <li key={u} className="px-3 py-2 flex items-center justify-between">
              ...
              <button onClick={()=>remove(u)} className="text-sm text-red-600 hover:underline">Remove</button>
            </li>
          ))}
        </ul>
      </div>
