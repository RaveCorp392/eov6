...
export default function OrgStaff({ orgId, org, onSaved, canManage = true }: Props) {
  ...
  async function invite() {
    ...
    onSaved({ ...org, users: Array.from(new Set([...(org.users ?? []), e])) });
    setEmail("");
    setBusy(false);
  }
  ...
  async function remove(u: string) {
    setBusy(true);
    await updateDoc(doc(db, "orgs", orgId), { users: arrayRemove(u) });
    onSaved({ ...org, users: (org.users ?? []).filter((x) => x !== u) });
    setBusy(false);
  }
