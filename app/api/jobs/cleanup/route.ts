...
export async function GET(req: Request) {
  ...
  // Auth with clear reasons + constant-time compare
  if (!envKey) {
    return NextResponse.json({ ok: false, error: "unauthorized", reason: "missing-env" }, { status: 401 });
  }
  if (!provided) {
    return NextResponse.json({ ok: false, error: "unauthorized", reason: "missing-key" }, { status: 401 });
  }
  if (!safeEqual(envKey, provided)) {
    return NextResponse.json({
      ok: false, error: "unauthorized", reason: "mismatch",
      envLen: envKey.length, providedLen: provided.length
    }, { status: 401 });
  }
  ...
  try {
    ...
    for (const doc of snap.docs) {
      ...
      if (!dryRun) {
        // Delete subcollections (v1 schema). If using `events`, add that too.
        counts.messages += await deleteSubcollection(`sessions/${code}/messages`);
        counts.fields += await deleteSubcollection(`sessions/${code}/fields`);
        // counts.messages += await deleteSubcollection(`sessions/${code}/events`);

        // Delete the session doc
        await doc.ref.delete();
        counts.sessions++;

        // Delete Storage folder uploads/{code}/**
    ...
    return NextResponse.json({
      ok: true,
      dryRun,
      expiredSessionsFound: snap.size,
      purgedCodes,
      counts,
      cap: SESSION_CAP,
...
async function deleteSubcollection(path: string): Promise<number> {
  ...
  let deleted = 0;
  while (true) {
    const page = await colRef.limit(BATCH_LIMIT).get();
    if (page.empty) break;
...
async function deleteStorageFolder(code: string): Promise<number> {
  ...
  for (let i = 0; i < files.length; i += BATCH_LIMIT) {
    ...
    await Promise.all(chunk.map(async (f) => {
      try { await f.delete(); }
      catch (e: any) { console.warn(`Delete failed for ${f.name}:`, e?.message || e); }
    }));
